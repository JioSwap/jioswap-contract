    ...
    IPoolRegistry
{
    ...
    bytes32 public constant JIO_MANAGER_ROLE =
        keccak256("JIO_MANAGER_ROLE");
    ...
    bytes32 public constant JIO_APPROVED_POOL_OWNER_ROLE =
        keccak256("JIO_APPROVED_POOL_OWNER_ROLE");
    ...
    constructor(address admin, address poolOwner) public payable {
        ...
        _setupRole(JIO_MANAGER_ROLE, msg.sender);
        _setupRole(JIO_APPROVED_POOL_OWNER_ROLE, poolOwner);
    }
    ...
    {
        require(
            hasRole(JIO_MANAGER_ROLE, msg.sender),
            "PR: Only managers can add pools"
        );
        ...
        managerOnly
    {
        ...
        // Effect
        poolData.isJioApproved = true;

        // Interaction
        require(
            hasRole(
                JIO_APPROVED_POOL_OWNER_ROLE,
                ISwap(poolAddress).owner()
            ),
            "Pool is not owned by jio"
        );
